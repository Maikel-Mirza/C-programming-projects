/*
  ...
  *****************************************************************************
*/
#define MAX_PATH_LENGTH 70 // max pathlength a game can have
#define MIN_PATH_LENGTH 10 // min pathlength a game can have
...
int main(void)
{
  ...
  int k = 1;                // flag
  /* these variables are used to print the board options that I want
  ...
  int maximumMoves = (double)game.pathLength * 75.0 / 100.0;
  do
  {
    ...
    fseek(stdin, 0, SEEK_END);
    if ((game.movesAllowed < player.lives) || (game.movesAllowed > maximumMoves))
    {
      // error message
      printf("    Value must be between %d and %d\n", player.lives, maximumMoves);
    }
    else
    {
      break;
    }
  } while ((game.movesAllowed < player.lives) || (game.movesAllowed > maximumMoves));
